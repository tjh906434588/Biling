...
/** 未接入 AI 模型时的全局提示横幅（放在工作台内容顶部，所有页面可见）。 */
export function AiNotReadyBanner({ onConfigure }: { onConfigure?: () => void }) {
  const { loading, ready, label } = useAiStatus();
  if (loading || ready) return null;
  return (
    <div className="mb-5 flex flex-wrap items-center gap-3 rounded-lg border border-red-300 bg-red-50 px-4 py-3 dark:border-red-900 dark:bg-red-950">
      <span aria-hidden className="h-2 w-2 shrink-0 rounded-full bg-red-500" />
      <div className="min-w-0 flex-1">
        <p className="text-sm font-medium text-red-700 dark:text-red-300">
          AI 模型未接入{label ? `（当前选中 ${label} 但未配置 Key）` : ""}
        </p>
        <p className="text-xs text-red-600/80 dark:text-red-300/80">
          没有可用的 AI 模型，所有 AI 功能（生成/提取/评价/大纲/蓝图/风格学习）暂不可用。
          点击右侧按钮前往「模型」页添加模型并填入 API Key。
        </p>
      </div>
      {onConfigure && (
        <button
          onClick={onConfigure}
          className="shrink-0 rounded-lg bg-red-600 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-red-500"
        >
          去配置模型
        </button>
      )}
